...
# ========================= manual-file review ============================
# The deterministic ingest rules the Review shares with the Validator live in
# ingest_rules.py, so both sections enforce one implementation (the Validator
# used to have no Publisher rules at all). The names below are local aliases;
# everything in this file uses them exactly as before.
import ingest_rules as _ir

_review_norm = _ir._review_norm
_url_equiv_key = _ir._url_equiv_key
_cat_is_publisher = _ir._cat_is_publisher
_row_is_publisher = _ir._row_is_publisher
_PUB_SUB_RE = _ir._PUB_SUB_RE
_PUB_BRAND_SET_RE = _ir._PUB_BRAND_SET_RE
_merge_brand_set = _ir._merge_brand_set
_BAD_FB_MANUAL_RE = _ir._BAD_FB_MANUAL_RE
_FANPAGE_RE = _ir._FANPAGE_RE
_pub_lines = _ir._pub_lines
_pub_val = _ir._pub_val
_pub_handle = _ir._pub_handle
_pub_dar_label = _ir._pub_dar_label
_pub_search_term_pair = _ir._pub_search_term_pair
publisher_review_findings = _ir.publisher_review_findings
PUBLICATION_TYPES = _ir.PUBLICATION_TYPES
PUBLISHER_REQUIRED_BRAND_SETS = _ir.PUBLISHER_REQUIRED_BRAND_SETS
PUBLISHER_BLANK_COLS = _ir.PUBLISHER_BLANK_COLS
PUB_LEAVE_BLANK = _ir.PUB_LEAVE_BLANK
PUB_CONFIRM_TYPE = _ir.PUB_CONFIRM_TYPE
_PUB_TYPE_PREFIX = _ir._PUB_TYPE_PREFIX
_PUB_TYPES_LOWER = _ir._PUB_TYPES_LOWER
_PUB_LI_URL_RE = _ir._PUB_LI_URL_RE
_PUB_VERIFIED_RE = _ir._PUB_VERIFIED_RE
_PUB_YT_RE = _ir._PUB_YT_RE
_PUB_WIKI_RE = _ir._PUB_WIKI_RE
_PUB_FB_URL_RE = _ir._PUB_FB_URL_RE
...
# ======================= Publishers: ingest rules =======================
# Learned from the 41-column BrandDefinitionReport / BrandIngest export
# (180-brand publishing file, Aug 2026). Every rule below holds for 100% of
# that file, so a violation is a real finding rather than a style difference.
#
#   title                        always '<publication> - DAR'
#   companies                    always 'Pristine Brand'
#   brand_set                    always contains 'LF // Publishing' AND
#                                'Pristine DAR Brands' (plus any curated sets)
#   title_sub_category           one or more 'Publication Type - X' lines,
#                                X from the 27-value dropdown below
#   twitter_search_terms         every line 'term|LABEL|LABEL'; a DAR row
#                                carries '@handle|DAR|DAR' + '#name|DAR|DAR'
#   twitter_search_term_keywords same 'term|LABEL|LABEL' shape (so a
#                                '#hashtag|DAR|DAR' keyword is NOT an error
#                                here, unlike the Movies/TV rule)
#   linkedin_page                'slug|LABEL' -- a slug, never a URL
#   pinterest_user_username      'slug|LABEL|LABEL'
#   pinterest_board              'Board Name|LABEL|LABEL' lines
#   facebook_verified            'TRUE|<url>' / 'FALSE|<url>', url matching
#   twitter_verified             facebook_page / twitter_handle
#   handles                      bare (no '@', no URL); instagram lowercase
#   youtube_channel_username     full youtube.com URL (@handle / user / channel)
#   wikipedia_page               en.wikipedia.org/wiki/... line(s)
#   genre / primary_genre /      always blank: a publication is not a rated
#   released_on / box office /   title and has no theatrical run
#   street_date / rottentomatoes
#   imdb_id / metacritic

# columns whose rules live in publisher_review_findings(); the discovery pass
# must not second-guess them
_PUB_RULE_COLS = set(PUBLISHER_BLANK_COLS) | {
    'title', 'title_category', 'title_sub_category', 'companies', 'brand_set',
    'twitter_search_terms', 'twitter_search_term_keywords', 'linkedin_page',
    'pinterest_user_username', 'pinterest_board', 'facebook_verified',
    'twitter_verified', 'twitter_handle', 'instagram_user', 'tiktok_user',
    'youtube_channel_username', 'youtube_channel_company', 'wikipedia_page',
    'facebook_page',
}
# columns the discovery pass may fill when the cell is empty (never a mismatch:
# for a publishing brand a curated value always wins)
_PUB_DISCOVERY_COLS = (
    'facebook_page', 'twitter_handle', 'instagram_user', 'tiktok_user',
    'youtube_channel_username', 'wikipedia_page', 'linkedin_page', 'network',
)
_PUB_HANDLE_BAD_RE = re.compile(r'https?://|\bwww\.|[/@]', re.I)
...
def _review_compare(col, manual_raw, expected_raw, title='', cat='',
    ...
    # This used to sit below the "expected empty -> pass" short-circuit, so it
    # only ran when discovery happened to return a genre -- with auto-discovery
    # off, or when discovery found none, a primary_genre that contradicts the
    # row's own genre column sailed through. It depends on nothing but the row,
    # so it is checked here (the Validator enforces the same rule offline).
    if c == 'primary_genre' and mval and _review_lines_ci(manual_genre) \
            and mval.strip().lower() not in _review_lines_ci(manual_genre):
        return False, sugg
...
def _tfx_schema_for_row(r, cat):
    ...
    if c.lower() in _TFX_LEGACY_CATS or 'game' in c.lower():
        return None
    # 'Publishers' is in the General master category list, but Publisher brands
    # have their OWN 40-column BrandDefinitionReport schema (create_publisher_row)
    # and must not be reviewed against the General brand rules.
    if _cat_is_publisher(c):
        return None
    key = _tfx_detect(r)
    if key:
        return key
...
def build_review(src, auto_fetch=True, progress=None):
    ...
    for i, r in enumerate(records):
        ...
        # ---- PUBLISHERS (40-col BrandDefinitionReport schema) ----
        # Publications are reviewed against the Publishers ingest RULES
        # (publisher_review_findings): DAR title, 'Pristine Brand' companies,
        # the LF // Publishing + Pristine DAR Brands sets, a
        # 'Publication Type - X' sub-category from the dropdown, '|DAR|DAR'
        # search terms / Pinterest / LinkedIn shapes, 'TRUE|url' verified
        # cells, and the columns a publication never carries (genre, box
        # office, RT / IMDb / Metacritic). Those rules do not need
        # auto-discovery; discovery only fills blank socials afterwards.
        if _row_is_publisher(r, cat, lower_cols):
            meta = dict(fetch_brand(t) or {}) if auto_fetch else {}
            # LinkedIn / Pinterest suggestions must follow the publisher shape
            if meta.get('linkedin_page'):
                meta['linkedin_page'] = _pub_dar_label(
                    _pub_handle(meta['linkedin_page']))
            if meta.get('pinterest_user_username'):
                meta['pinterest_user_username'] = _pub_dar_label(
                    _pub_handle(meta['pinterest_user_username']), 2)

            rule_findings = publisher_review_findings(
                r, lower_cols, t, cat, meta)
            # every rule counts as a checked cell, plus the discovery columns
            cells_checked += len(_PUB_RULE_COLS & set(lower_cols)) \
                + len([c for c in _PUB_DISCOVERY_COLS if c in lower_cols])
            for fd in rule_findings:
                if fills.get((i, fd['column'])):
                    continue
                findings.append(dict(row=i + 2, title=t, column=fd['column'],
                                     status=fd['status'], current=fd['current'],
                                     suggested=fd['suggested']))
                fills[(i, fd['column'])] = fd['status']

            # discovery pass: fill EMPTY socials only -- for a publishing brand
            # a curated value always wins, so this never raises a Mismatch
            if meta:
                for col in _PUB_DISCOVERY_COLS:
                    src_col = lower_cols.get(col)
                    if src_col is None or fills.get((i, src_col)):
                        continue
                    if _review_norm(r.get(src_col)):
                        continue
                    sugg = str(meta.get(col) or '').strip()
                    if col == 'facebook_page' and sugg and (
                            _BAD_FB_SUGG_RE.search(sugg) or _FANPAGE_RE.search(sugg)):
                        sugg = ''
                    if not sugg:
                        continue
                    findings.append(dict(row=i + 2, title=t, column=src_col,
                                         status='Gap', current='',
                                         suggested=sugg))
                    fills[(i, src_col)] = 'Gap'
            if progress:
